...
if __name__ == "__main__":
    ...
    model = MJ.MjModel.from_xml_path("../models/world_6_general.xml")
    ...
    width=640,
    height=480,
    layout=GridLayout(np.ceil(np.sqrt(NWORLD)).astype(int), np.ceil(np.sqrt(NWORLD)).astype(int))
    )
    
    recorder.open("osc_warp_test.mp4")
